...
def Config(net, args):
    ...
    AB.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)
    BA.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)

    BC = B.intf(B.name+'-eth1')
    CB = C.intf(C.name+'-eth0')

    BC.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)
    CB.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)

    BD = B.intf(B.name+'-eth2')
    DB = D.intf(D.name+'-eth0')

    BD.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)
    DB.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)

    CD = C.intf(C.name+'-eth1')
    DC = D.intf(D.name+'-eth1')

    CD.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)
    DC.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)

    DE = D.intf(D.name+'-eth2')
    ED = E.intf(E.name+'-eth0')

    DE.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)
    ED.config(bw=10, delay='200ms', loss=10.0, use_tbf=False)
    ...
    E.cmd('xterm -T "TP E" -e ./transport_daemon -d 1 appE tpE &')
    sleep(1)
    E.cmd('xterm&')
    E.cmd('xterm&')
    E.cmd('xterm')
