...
def Config(net, args):
    ...
    A.cmd('xterm -T "HOST A" &')
    sleep(1)
    B.cmd('xterm -iconic -T "MIP B"  -e ./mip_daemon -d app2 route2 fwd2 20 30 &')
    B.cmd('xterm -iconic -T "Route B"  -e ./routing_daemon -d route2 fwd2 &')
    B.cmd('xterm -T "TP B"  -e ./transport_daemon -d 1 app2 tp2 &')
    B.cmd('xterm -T "HOST B" &')
    sleep(1)
    C.cmd('xterm -iconic -T "MIP C"  -e ./mip_daemon -d app3 route3 fwd3 40 &')
    C.cmd('xterm -iconic -T "Route C"  -e ./routing_daemon -d route3 fwd3 &')
    C.cmd('xterm -T "TP C"  -e ./transport_daemon -d 1 app3 tp3 &')
    C.cmd('xterm -T "HOST C" ')
    sleep(1)
